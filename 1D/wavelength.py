...
class TissueStrand:
    def __init__(self, cellmodel, D, L=200, h=0.5, plot_args={},
                 stim_amp=-1410*8, scpath="../data/steadycycles/"):
        self.D = Constant(D)
        self.L = L
        self.h = h
        ...
        self.ode = cellmodel
        self.cellmodel = load_ode(cellmodel)
        cellmodel = dolfin_jit(self.cellmodel, field_states=['V'],
    ...
    def wavelength(self, BCL, dt=0.1, num_of_pulses=3, threshold=0.5, liveplot=False):
        ...
        for i in range(num_of_pulses):
            wavelength = 0
            for timestep, (u, vm) in self.solver.solve((i*BCL, (i+1)*BCL), dt):
                if liveplot: plot(u, **self.plot_args)

                if not wavelength:
                    vmax = np.max(u.vector().array()) 
                    vmin = np.min(u.vector().array())
                    vthresh = vmin + 0.1*(vmax - vmin)
                    uvec = u.vector().array()
                    if uvec[-1] < vthresh and uvec[0] < vthresh:
                        wavelength = len(uvec[uvec > vthresh])*h
                        print wavelength
...
if __name__ == '__main__':
    ...
    solver = TissueStrand('hAM_KSMT_nSR', 0.40, stim_amp=-1410*8, plot_args={'range_min':-80.0, 'range_max':40.0})
    #solver = TissueStrand('hAM_KSMT_cAF', 0.31, stim_amp=-1410*8, threshold=0, plot_args={'range_min':-80.0, 'range_max':40.0})
    #solver = TissueStrand('FK_nSR', 0.077, stim_amp=-0.8, plot_args={'range_min':0.0, 'range_max':1.0})
    #solver = TissueStrand('FK_cAF', 0.077, stim_amp=-0.8, threshold=0.5, plot_args={'range_min':0.0, 'range_max':1.0})

    solver.wavelength(1000, dt=0.1, num_of_pulses=5, liveplot=True)
